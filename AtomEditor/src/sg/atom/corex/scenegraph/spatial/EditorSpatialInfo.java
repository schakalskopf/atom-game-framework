 ...
 * @author hungcuong
 */
public class EditorSpatialInfo extends SpatialInfo implements Selectable {

    private SpatialPresentor presentor;
    private EditorSpatialInfo parentInfo;
    private int type;
    private SpatialList children = new SpatialList();

    public EditorSpatialInfo(Spatial spatial, EditorSpatialInfo parentInfo) {
        ...
        if (CommonTool.getDefault(null).getRegistered(HelperManager.class).isHelperNode(spatial)) {
            ...
            if (spatial instanceof Node) {
                ...
                for (Spatial sp : ((Node) spatial).getChildren()) {
                    EditorSpatialInfo child = new EditorSpatialInfo(sp, this);
                    children.add(child);
                }
            } else {
                this.type = 1;
