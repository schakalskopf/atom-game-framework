package sg.atom2d.geometry;

/**
 * A <em>Region</em> is simply an area, as the name implies, and is implemented
 * as a so called "y-x-banded" array of rectangles; Each Region is made up of a
 * certain number of rectangles sorted by y coordinate first, and then by x
 * coordinate.
 *
 * <p> Furthermore, the rectangles are banded such that every rectangle with a
 * given upper-left y coordinate (y1) will have the same lower-right y
 * coordinate (y2) and vice versa. If a rectangle has scanlines in a band, it
 * will span the entire vertical distance of the band. This means that some
 * areas that could be merged into a taller rectangle will be represented as
 * several shorter rectangles to account for shorter rectangles to its left or
 * right but within its "vertical scope".
 *
 * <p> An added constraint on the rectangles is that they must cover as much
 * horizontal area as possible. E.g. no two rectangles in a band are allowed to
 * touch. Whenever possible, bands will be merged together to cover a greater
 * vertical distance (and thus reduce the number of rectangles). Two bands can
 * be merged only if the bottom of one touches the top of the other and they
 * have rectangles in the same places (of the same width, of course). This
 * maintains the y-x-banding. <p> Region operations includes add (union),
 * subtract, intersect, and exclusive-or. <p> This class corresponds to Region.c
 * of the X11 distribution and the implemntation is based on it. <p> The
 * <em>Region</em> is essentially equivalent to an AWT <em>Area</em> but with
 * different back-end implementation. Becnhmarking proves it more than 100 times
 * faster than AWT Area for binary CAG operations, <p>
 *
...
public class RectRegion
     ...
     */
    public RectRegion() {
        extent_ = new BoxInt(0, 0, 0, 0);
        rectangles_ = new int[INITIAL_SIZE];
        nRectangles_ = 0;
    }

    /**
     * Create the region covering the (possibly complex) polygon specified by
     * the x and y parameters. Corresponds to XPolygonRegion of X11.
     *
     * @param x X values of polygon vertices.
     * @param y Y values of polygon vertices.
     */
    public RectRegion(int x[], int y[]) {
     ...
     */
    public RectRegion(RectangleInt rectangle) {
        this();
        set(rectangle);
    }
    ...
    public RectRegion(int x, int y, int width, int height) {
        this(new RectangleInt(x, y, width, height));
    }
     ...
     */
    public RectRegion(BoxInt box) {
        this(new RectangleInt(box));
    }
     ...
     */
    public RectRegion(RectRegion region) {
        extent_ = new BoxInt();
    ...
    /**
     * Ensure memory for the rectangles.
     *
     * @param region
     * @param nRectangles
     */
    private static void checkMemory(RectRegion region, int nRectangles) {
     ...
     * @param region Region to copy.
     */
    public void set(RectRegion region) {
     ...
     */
    public void set(RectangleInt rectangle) {
        rectangles_ = new int[INITIAL_SIZE];
    ...
    /**
     * Return true if this region equals the specified object. Corrensponds to
     * XEqualRegion of X11.
     *
    ...
    public boolean equals(Object object) {
        RectRegion region = (RectRegion) object;

        if (nRectangles_ != region.nRectangles_) {
            return false;
        } else if (nRectangles_ == 0) {
            return true;
        } else if (extent_.x1 != region.extent_.x1) {
            return false;
        } else if (extent_.x2 != region.extent_.x2) {
            return false;
        } else if (extent_.y1 != region.extent_.y1) {
            return false;
        } else if (extent_.y2 != region.extent_.y2) {
            return false;
        } else {
            for (int i = 0; i < nRectangles_ << 2; i++) {
                if (rectangles_[i] != region.rectangles_[i]) {
                    return false;
                }
            }
        }
    ...
    /**
     * Offset the entire region a specified distance. Corresponds to
     * XOffsetRegion in X11.
     *
     ...
     */
    public boolean isIntersecting(RectRegion region) {
        RectRegion r = (RectRegion) clone();
        r.intersect(region);
        return !r.isEmpty();
    }
     ...
     */
    public boolean isIntersecting(RectangleInt rectangle) {
        RectRegion region = new RectRegion(rectangle);
        return isIntersecting(region);
    }

    /**
     * Return true if the specified point is inside this region. This method
     * corresponds to XPointInRegion in X11.
     *
    ...
    /**
     * Return true if the specified rectangle is inside this region. This method
     * corresponds to XRectInRegion in X11.
     *
     * @param rectangle Rectangle to check.
     * @return True if the rectangle is inside this region, false otherwise.
     */
    public boolean isInside(RectangleInt rectangle) {
        ...
        while (i < rEnd) {
            ...
            if (x1 < rectangles_[i + 2] || x2 > rectangles_[i + 3]) {
                return false;
            }

            // See if we are done
            if (rectangles_[i + 1] >= y2) {
                return true;
            }
     ...
     * @return True if this region is inside the specified rectangle, false
     * otherwise.
     */
    public boolean isInsideOf(RectangleInt rectangle) {
     ...
     */
    public RectangleInt getExtent() {
        return new RectangleInt(extent_);
    }

    /**
     * Return the number of rectangles in the region. In case the number is
     * getting very high, the application might choose to call collapse().
     *
    ...
    /**
     * Perform a logical set operation between this and the specified region.
     * Corresponds to miRegionOp in Region.c of X11.
     *
     * @param region Region to combine with.
     * @param operationType Combination operator.
     */
    private void combine(RectRegion region, int operationType) {
        ...
        do {
            ...
            if (rectangles_[r1 + 1] == yBottom) {
                r1 = r1BandEnd;
            }
            if (region.rectangles_[r2 + 1] == yBottom) {
                r2 = r2BandEnd;
            }
    ...
    private void nonOverlap1(int rectangles[], int r, int rEnd,
        ...
        if (operationType == OPERATION_UNION
            ...
            while (r != rEnd) {
                ...
                gRectangles_[i] = yTop;
                i++;
                gRectangles_[i] = yBottom;
                i++;
                gRectangles_[i] = rectangles[r + 2];
                i++;
                gRectangles_[i] = rectangles[r + 3];
                i++;
                gNRectangles_++;
                r += 4;
            }
        ...
        //
            ...
            while (r1 != r1End && r2 != r2End) {
                ...
                if (rectangles1[r1 + 3] < rectangles2[r2 + 3]) {
                    r1 += 4;
                } else if (rectangles2[r2 + 3] < rectangles1[r1 + 3]) {
                    r2 += 4;
                } else {
                    r1 += 4;
                    r2 += 4;
    ...
    private void updateExtent() {
        if (nRectangles_ == 0) {
            ...
            for (int i = 4; i < nRectangles_ << 2; i += 4) {
                if (rectangles_[i + 2] < extent_.x1) {
                    extent_.x1 = rectangles_[i + 2];
                }
                if (rectangles_[i + 3] > extent_.x2) {
                    extent_.x2 = rectangles_[i + 3];
                }
            }
        }
    }
    ...
    /**
     * Union this region with the specified region. Corresponds to XUnionRegion
     * in X11.
     *
     * @param region Region to union this with.
     */
    public void union(RectRegion region) {
    ...
    /**
     * Union this region with the specified rectangle. Corresponds to
     * XUnionRectWithRegion in X11.
     *
    ...
    public void union(RectangleInt rectangle) {
        ...
        union(new RectRegion(rectangle));
    }
     ...
     */
    public static RectRegion union(RectRegion r1, RectRegion r2) {
        RectRegion region = new RectRegion(r1);
        region.union(r2);
        return region;
    }
     ...
     */
    public static RectRegion union(RectRegion region, RectangleInt rectangle) {
        if (rectangle.isEmpty()) {
            return new RectRegion(region);
        } else {
            return union(region, new RectRegion(rectangle));
        }
    }

    /**
     * Leave this region as the intersection between this region and the
     * specified region. Corresponds to XIntersectRegion in X11.
     *
     * @param region Region to intersect this with.
     */
    public void intersect(RectRegion region) {
    ...
    public void intersect(RectangleInt rectangle) {
        if (rectangle.isEmpty()) {
            ...
            intersect(new RectRegion(rectangle));
        }
    }
     ...
     */
    public static RectRegion intersect(RectRegion r1, RectRegion r2) {
        RectRegion region = new RectRegion(r1);
        region.intersect(r2);
        return region;
    }
     ...
     */
    public static RectRegion intersect(RectRegion region, RectangleInt rectangle) {
        if (rectangle.isEmpty()) {
            return new RectRegion();
        } else {
            return intersect(region, new RectRegion(rectangle));
        }
    }

    /**
     * Subtract the specified region from this region. Corresponds to
     * XSubtractRegion in X11.
     *
     * @param region Region to subtract from this region.
     */
    public void subtract(RectRegion region) {
    ...
    public void subtract(RectangleInt rectangle) {
        ...
        subtract(new RectRegion(rectangle));
    }
     ...
     */
    public static RectRegion subtract(RectRegion r1, RectRegion r2) {
        RectRegion region = new RectRegion(r1);
        region.subtract(r2);
        return region;
    }
     ...
     * @return Subtraction of the two specified regions.
     */
    public static RectRegion subtract(RectRegion region, RectangleInt rectangle) {
        if (rectangle.isEmpty()) {
            return new RectRegion(region);
        } else {
            return subtract(region, new RectRegion(rectangle));
        }
    }

    /**
     * Leave the exclusive-or between this and the specified region in this
     * region. Corresponds to the XXorRegion in X11.
     *
     * @param region Region to xor this region with.
     */
    public void xor(RectRegion region) {
        RectRegion r = (RectRegion) region.clone();
        r.subtract(this);
        subtract(region);
        union(r);
    }
    ...
    /**
     * Leave the exclusive-or between this and the specified rectangle in this
     * region.
     *
     ...
     */
    public static RectRegion xor(RectRegion r1, RectRegion r2) {
        RectRegion region = new RectRegion(r1);
        region.xor(r2);
        return region;
    }
    ...
    /**
     * Do an exlusive-or operation between a regions and a rectangle and return
     * the result.
     *
     ...
     * @return Result of operation.
     */
    public static RectRegion xor(RectRegion region, RectangleInt rectangle) {
        if (rectangle.isEmpty()) {
            return new RectRegion();
        } else {
            return xor(region, new RectRegion(rectangle));
        }
    }
    ...
    private boolean isExtentCorrect() {
        ...
        if (nRectangles_ > 0) {
            ...
            xMax = rectangles_[3];
            for (int i = 4; i < nRectangles_ << 2; i += 4) {
                if (rectangles_[i + 0] < yMin) {
                    yMin = rectangles_[i + 0];
                }
                if (rectangles_[i + 1] > yMax) {
                    yMax = rectangles_[i + 1];
                }
                if (rectangles_[i + 2] < xMin) {
                    xMin = rectangles_[i + 2];
                }
                if (rectangles_[i + 3] > xMax) {
                    xMax = rectangles_[i + 3];
                }
            }
        }
