...
import sg.atom.core.lifecycle.ManagedObject;
import sg.atom.core.lifecycle.ManagedObjectRemoval;
import sg.atom.core.lifecycle.ManagedReference;
import sg.atom.core.lifecycle.ObjectNotFoundException;
 ...
 * to directly or indirectly, are stored by the
 * <code>DataManager</code>. It is up to the application to determine when
 * managed objects are no longer needed and to remove them explicitly from the
 * <code>DataManager</code> using the {@link #removeObject removeObject} method.
 * <p>
 *
 ...
 * manager and are not stored persistently. <p>
 *
 * Passing a transient object to {@code setBinding}, {@code
 *     createReference}, or {@code getObjectId} creates a new entry for the object
 * in the data manager. Calling {@code removeObject} or {@link
 *     #markForUpdate markForUpdate} on a transient object does not change the state
 * of the object. If a transient object implements {@link
 *     ManagedObjectRemoval}, though, then calling {@code removeObject} will cause
 * its {@link ManagedObjectRemoval#removingObject removingObject} method to be
 * called. <p>
 *
 ...
 *     createReference}, or {@code getObjectId} in the current transaction, or else
 * retrieved from the data manager by calling {@link #getBinding
 ...
 * create a new entry for that object in the data manager, but rather reuses the
 * existing one. Calling {@code removeObject} on a persistent object removes it
 * from the data manager, meaning it will no longer be stored persistently, and
 * changes its state to <i>removed</i>. Note that a managed object made
 * persistent in the current transaction is automatically marked for update, so
 * there is no need to call {@code
 *     markForUpdate} for such objects, in particular in their constructors. <p>
 *
 ...
 * effort to track which managed objects have been removed. <p>
 *
 * Only persistent managed objects can become removed; calling {@code
 *     removeObject} on a transient managed object leaves it as a transient object.
 * Passing a removed managed object to {@code setBinding}, {@code
 ...
 *     ManagedReference.getForUpdate} on a reference to the removed object, will
 * cause a {@link ObjectNotFoundException} to be thrown, if the system has
 * tracked the object's removed state. <p>
 *
 ...
 * well as calling {@code ManagedReference.get} or {@code
 *     ManagedReference.getForUpdate} on a reference to the stale object, will cause
 * a {@link TransactionNotActiveException} to be thrown, if the system has
 * tracked the object's stale state. <p>
 *
 * </ul> <p>
 *
...
public interface DataManager {
     ...
     * referred to objects implement
     * <code>Serializable</code>. Any instances of {@link ManagedObject} that
     * <code>object</code> refers to directly, or indirectly through non-managed
     * objects, need to be referred to through instances of
     * {@link ManagedReference}.
     *
     * @param	name the name
     ...
     * the object is removed. To remove the object, use the {@link
     ...
     * <code>name</code> is
     * <code>null</code>, then the search starts at the beginning. <p>
     *
     ...
     * <code>DataManager</code>, if the object is <a
     * href="#persistent">persistent</a>. The system will make an effort to flag
     * subsequent references to the removed object through {@link
     ...
     * <a href="#transient">transient</a>, then this method first calls the      {@link ManagedObjectRemoval#removingObject
     ...
     *		ManagedObjectRemoval} and {@code removeObject} is called recursively on
     * the object through a call to {@link
     ...
     *		ManagedObjectRemoval#removingObject
     *		ManagedObjectRemoval.removingObject} on the object throws a runtime
     * exception
