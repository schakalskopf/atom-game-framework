...
/**
 * An interface for doing operations after some delay. Allows expiration to
 * occur on a specific thread, and guarantees that any queued expiration will
 * not run if the Interval has since been cancelled or rescheduled.
 */
public abstract class Interval {

    /**
     * A marker {@link RunQueue} to supply when you intend for your interval to
     * run directly on the interval timer thread, rather than having the
     * interval processed on a separate thread. Warning: you must be absolutely
     * sure your interval will complete <em>very quickly</em>, otherwise you
     * risk delaying the firing of other intervals.
     */
    ...
    /**
     * An interface for entities that create, and keep track of, intervals. The
     * intended use case is for repeating intervals to be created via a factory
     * that tracks all such intervals, and which automatically cancels still
     * running intervals when the factory is shut down.
     */
    public interface Factory {

        /**
         * Creates an {@link Interval} that runs the supplied runnable. For
         * example:
         * <pre>
    ...
    /**
     * An interface that will be implemented by the runnable posted to a
     * RunQueue that can be used to retrieve the original Interval.
     */
    public static interface RunBuddy extends Runnable {

        /**
         * Retrieve the Interval that is responsible for posting this RunBuddy
         * to a RunQueue. Most likely used to call toString() on the Interval
         * for logging purposes.
         */
        public Interval getInterval();

        /**
         * Returns the class name of the interval (valid even if the interval
         * has been cancelled and its reference cleared).
         */
        public String getIntervalClassName();
    }

    /**
     * Creates an interval that executes the supplied runnable on the specified
     * RunQueue when it expires.
     */
    ...
    /**
     * @deprecated If direct-running is desired, pass {@link #RUN_DIRECT}
     * explicitly.
     */
    @Deprecated
    public Interval() {
    ...
    /**
     * Create an Interval that uses the specified {@link RunQueue} to run the
     * {@link #expired} method. If null is supplied the interval will be run
    ...
    /**
     * Schedules this interval to execute once at
     * <code>when</code>. Supersedes any previous schedule that this Interval
     * may have had.
     */
    ...
    /**
     * Schedule the interval to execute repeatedly with fixed-rate scheduling
     * between repeats, with the same delay. Supersedes any previous schedule
     * that this Interval may have had.
     *
     * <p> Note: if a repeating interval is scheduled to post itself to a
     * RunQueue and the target RunQueue is shutdown when the interval expires,
     * the interval will cancel itself and log a warning message. </p>
     */
    ...
    /**
     * Schedule the interval to execute repeatedly with the specified initial
     * delay and repeat delay with fixed-rate scheduling between repeats.
     * Supersedes any previous schedule that this Interval may have had.
     *
     * <p> Note: if a repeating interval is scheduled to post itself to a
     * RunQueue and the target RunQueue is shutdown when the interval expires,
    ...
    /**
     * Schedule the interval to execute repeatedly with the specified initial
     * delay and repeat delay. Supersedes any previous schedule that this
     ...
     * the interval will cancel itself and log a warning message. </p>
     *
     * @param fixedRate - if true, this interval schedules repeated expirations
     * using {@link Timer#scheduleAtFixedRate(TimerTask, long, long)} ensuring
     * that the number of expired calls will match the amount of time elapsed.
     * If false, it uses {@link Timer#schedule(TimerTask, long, long)} which
     * ensures that there will be close to <code>repeateDelay</code>
     * milliseconds between expirations.
     *
     * @exception IllegalArgumentException if fixedRate is false and a RunQueue
     * has been specified. That doesn't make sense because the fixed delay
     * cannot account for the time that the RunBuddy sits on the RunQueue
     * waiting to call expire().
     */
    ...
    /**
     * Cancel the current schedule, and ensure that any expirations that are
     * queued up but have not yet run do not run.
     */
    ...
    /**
     * Note (log) that we were unable to be posted to our RunQueue because it is
     * no longer running.
     */
    protected void noteRejected() {
        //log.warning("Interval posted to shutdown RunQueue. Cancelling.","queue", _runQueue, "interval", this);
    }
    ...
    protected static class IntervalTask extends TimerTask {
        ...
        public void run() {
            ...
            if (ival._runQueue.isRunning()) {
                try {
                    ...
                    //log.warning("Failed to execute interval on run-queue","queue", ival._runQueue, "interval", ival, e);
                }
        ...
        /**
         * The interval this task is for. We have this reference back to our
         * interval rather than just being a non-static inner class because when
         * a TimerTask is cancelled it still sits on the Timer's queue until its
         * execution time is reached. We want any references held by the
         * interval to be collectable during this period, so our cancel removes
         * the reference back to the Interval.
         */
        protected Interval _interval;
        /**
         * The class name of the interval (so that we can identify it after
         * cancellation).
         */
        protected String _intervalClassName;
    } // end: static class IntervalTask
    /**
     * The RunQueue used to run the expired() method for this Interval, or
     * {@link #RUN_DIRECT} to indicate that the interval should be executed
     * directly on the Inteval timer thread.
     */
