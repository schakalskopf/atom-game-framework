...
//import sg.atom.utils.space.Field;
//import sg.atom.utils.space.Tuple;
//import sg.atom.utils.space.interfaces.IField;
import sg.atom.utils.repository.space.interfaces.ITuple;
import sg.atom.utils.repository.space.interfaces.ITupleSpace;
import sg.atom.utils.repository.space.interfaces.TupleSpaceException;
//import sg.atom.utils.space.XMLField;
...
/**
 * Abstract "Query"-based worker. The idea of this worker is that it has a list
 * of templates for tuples that it is "interested in", and for which it
 * associates a process. Implementations of this worker should be built with a
 * list of templates (using the method "addquerytemplate"), and should implement
 * the abstract method "answerqueries" to process the collected tuples.
 *
 * Basic usage: 1) define a list of templates and their associated processing 2)
 * add each template to the worker using addQueryTemplate(ITuple) 3) define the
 * processing in abstract method answerQueries(ITuple template, Ituple query) 4)
 * start the worker using method start() 5) if applicable, stop the agent
 * whenever necessary 6) during normal running of the agent, it is possible to
 * temporarily listen for the asynchronous arrival of certain tuples, using a
 * listener pattern. Use the addQueryListener method.
 */
public abstract class AbstractWorker extends Worker {
    ...
    /**
     * lists to manage the tuples that have already been read they are indexed
     * by template (i.e. for each template / query accepted by the worker, there
     * is a list)
     *
     */
    /**
    ...
    /**
     * Threads that read the tuplespace waiting for new tuples They are mapped
    ...
    /**
     * Abstract method for answering "queries" (generic processing associated to
     * particular tuples).
     *
     * @param template the template that was used to collect the tuple
     * @param query the tuple
     * @return a possibly empty list of tuples to be placed in the tuple space
     * as a result of the processing, e.g. "answers" to the query
     */
    ...
    /**
     * Stop this worker. TODO: make sure that everything is stopped properly
     */
    public void stop() {
        super.stop();
    ...
    /**
     * adds a tuple to the list of 'done' queries. The idea is that this should
     * be done to avoid the worker outputting a query and attempting to process
     * it himself
     *
    ...
    protected void ignoreQuery(ITuple template, ITuple query) {
        ...
        if (key != null && templateScanners.containsKey(key)) {
            ...
            LOG.log(Level.INFO, name + " added query to be ignored : " + query);
        } else {
            LOG.log(Level.INFO, name + " doesn't have SEARCH among his templates ");
        }
    }

    /**
     * Answer queries. Go through the supported templates, check for a matching
     * query, and answer it
     *
     */
    private void answerQueries() {
        ...
        LOG.log(Level.INFO, name + " processing query:" + querypair.query);
        ...
        if (ansTuple != null && //don't think it will be null... but it could be empty.
            ...
            try {
                ...
                LOG.log(Level.SEVERE, "TupleSpace Error:" + e);
            }
            //}
            LOG.log(Level.INFO, name + " worker posting " + ansTuple.size() + " answers for query " + querypair.query.toString());
        } else {
            LOG.log(Level.INFO, name + ": No immediate answer for query " + querypair.query.toString());
        }
    ...
    protected void addListener(TupleListener listener, ITuple template) {
        ...
        LOG.log(Level.INFO, "returning from launching an async search");
    }

    /**
     * Method to do synchronous queries on the tuple space -- output query, wait
     * for answer this one should be used to retrieve multiple answers
     *
     ...
     * @throws TupleSpaceException for whatever reason TupleSpace.in(ITuple) may
     * throw an exception
     */
    protected List<ITuple> synchronousMultiQuery(ITuple query, ITuple answerTemplate) throws TupleSpaceException {
        ...
        if (answer == null) {
            LOG.log(Level.SEVERE, "syncMultiQuery got a null answer!");
    ...
    /**
     * Method to do asynchronous queries on the tuple space -- output query,
     * register listener
     *
     * @param query a tuple defining the query
     * @param answerTemplate a tuple template defining the answer
    ...
    /**
     * outputs a tuple [ "Error", exception] so the exception can be handled by
     * user interface
     *
    ...
    /**
     * Work: normal processing of this worker. The principle of such a worker is
     * that it has a standard list of templates that it is listening for, and
     * associated processing for these tuples. Thus the standard processing by
     * this agent is to process the tuples using the abstract method
     * "answerqueries". This method should be customized for the worker.
     */
    public void work() {
    ...
    //////////////////////////////////
    /**
     * this class is a thread that scans the tuplespace with a specific
     * template, and drops the tuples to be processed in an "Inbox" object,
     * which is a message queue
     *
     * @author alan
     *
     */
    private class TSScanner extends Thread {
        ...
        /**
         * Collects tuples matching the input template. If these tuples have
         * been recently read, then we drop them field 0 is a string "Search",
         * field 1 is a formal field of class "SearchString"
         *
        ...
        protected List<ITuple> scanForQueries(boolean checkDuplicates) {
            ...
            //LOG.log(Level.INFO,name+ " worker looking for work...");

            try {
                LOG.log(Level.INFO, name + "/" + template.get(0) + " looking for work...");
                ...
                if (allqueries == null) { //if there's no matching tuples, then we want to wait rather than looping
                    ...
                    doneList.clear();
                    LOG.log(Level.INFO, name + "/" + template.get(0) + "going into WAIT mode...");
                    space.rd(template); //wait until one matching tuple shows up (do not save read tuple since we'll get it right after)
                    LOG.log(Level.INFO, name + ": scanner " + template + " reading something!!");
            ...
            if (allqueries != null) { //if we didn't find any queries we may as well leave!
                ...
                for (ITuple q : allqueries) {
                    ...
                    if (readqueries != null && readqueries.contains(qt)) //check if we've had this query recently
                    {
                        ...
                        //LOG.log(Level.INFO,name+" ignoring tuple:"+qt);
        ...
        /**
         * run: scan the tuplespace for tuples matching the template, filter out
         * those that have been seen recently, and place the remaining ones to
         * the "inbox" of this worker. Loop until the thread is told to shutdown
         * using the mystop() method
         *
        ...
        /**
         * stop this thread (could not call it stop because that's a deprecated
         * & final method the thread will not stop immediately but finish one
         * iteration of scanning for queries, etc..
         */
        public void mystop() {
            shutdownflag = true;
        }
    ...
    /////////////////////////////////////////////////////////////////////////////////////////
    /**
     * A tuplePair is a couple of tuples (template, tuple), where template
     * should
     * <code>match</code> tuple. It is used in the queue of tuples to be
     * processed by the worker. Processing tasks are associated to the different
     * templates of a worker, and as the matching tuples are collected by
     * templates, this structure allows new tasks in the queue to appear as a
     * task (represented by the template) associated to the tuple.
     *
     * @author alan
     */
    ...
    /**
     * Inbox is a class that implements a synchronized message queue, of the
     * producer / consumer pattern. Producers are TS Scanners, and the consumer
    ...
    private class Inbox {
        ...
        /**
         * Queue of tuples/templates (TuplePair) in the order that the tuples
         * arrive in the tuplespace
         *
         */
        private LinkedList<TuplePair> todoQueue;

        /**
         * constructor. All that is needed is a linked list for the message
         * queue
         */
        public Inbox() {
        ...
        public synchronized TuplePair get() {
            while (todoQueue.isEmpty()) {
                try {
                    ...
                    wait();
                    LOG.log(Level.INFO, name + " waking up!");
                } catch (InterruptedException e) {
                }
            }
...
//////////////////////////////////////////////////////////////////////////////////
    /**
     * a query listener that can be added in order to collect asynchronous
     * search responses and do callbacks to the objects who placed the searches
     * (or their specified listeners)
     */
    private class QueryListener implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                if (alltuples == null) { //if there's no matching tuples, then we want to wait rather than looping
                    space.rd(template); //wait until one matching tuple shows up (do not save read tuple since we'll get it right after)
                    LOG.log(Level.INFO, name + ": listener for search " + template + " getting async answers!!");
        ...
        /**
         * Extract the results found in the TS and send them back to the
         * listener as SearchResponse[] TODO: replace all this by simply sending
         * the list of tuples. The subscriber can manage that
         *
         * @param res
         */
