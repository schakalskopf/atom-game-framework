...
import sg.atom.utils.repository.cache.CacheExpiry;
import sg.atom.utils.repository.cache.ReadOnlyRevisionObjectCache;
import sg.atom.utils.repository.cache.RootObjectCache;
import sg.atom.utils.repository.cache.ObjectCacheException;
import sg.atom.utils.repository.cache.CacheElementFactory;
import sg.atom.utils.repository.cache.CacheElementRevisions;
import sg.atom.utils.repository.cache.RevisionObjectCacheMap;
import sg.atom.utils.repository.pojomvcc.*;
...
/**
 * A {@code RootObjectCache} is the root of the {@code ObjectCache} cache
 * heirarchies.
 * <p/>
 * A {@code RootObjectCache} is not modifyable directly, a
 * {@code RevisionObjectCache} must be created (using the
 * {@link RootObjectCacheImpl#checkout()} method) and the changes made to that.
 *
 * @author Aidan Morgan
 */
public class RootObjectCacheImpl<K, V> implements RootObjectCache<K, V> {
    ...
    /**
     * The {@code java.util.concurrent.locks.ReadWriteLock} that ensures safe
     * access to this cache from multiple threads.
     */
    private final ReadWriteLock readWriteLock = new ReentrantReadWriteLock();
    /**
     * A {@code List} of currently active
     * {@code sg.atom.utils.repository.pojomvcc.RevisionObjectCache}s.
     */
    private List<RevisionObjectCache<K, V>> openRevisionCaches;
    /**
     * A {@code java.util.Map} that stores a {@code K} with the
     * {@code sg.atom.utils.repository.pojomvcc.impl.ObjectRevisions} that track
     * the history of the objects.
     */
    private Map<K, CacheElementRevisions<K, V>> revisions;
    /**
     * A {@code java.util.Map} of revision to a {@code java.util.List} of
     * {@link K} that are the keys in the
     * {@code sg.atom.utils.repository.pojomvcc.RootObjectCache} for that
     * revision.
     */
    private Map<Long, RevisionKeyList<K>> keysPerRevisionMap;
    /**
     * The {@link CacheElementFactory<V>} that is used for creating clones and
     * merging {@link V}s.
     */
    private CacheElementFactory<V> factory;
    /**
     * The {@link sg.atom.utils.repository.pojomvcc.CacheExpiry} that determines
     * how long historical information about {@link V}s should be retained. It
     * also can provide a mechanism for handling expired
     * {@link sg.atom.utils.repository.pojomvcc.RevisionKeyList}s so that that
     ...
     * Constructor.
     *
     * @param factory the {@link CacheElementFactory<V>} that is used for
     * creating clones and merging {@link V}s.
     */
    ...
    /**
     * Handles the expiry of
     * {@link sg.atom.utils.repository.pojomvcc.RevisionKeyList}s from this
     * {@link sg.atom.utils.repository.pojomvcc.RootObjectCache}. Will use the
     * {@link sg.atom.utils.repository.pojomvcc.CacheExpiry} registered to check
     * which revisions should be evicted from memory.
     */
    private void expire() {
        try {
            readWriteLock.writeLock().lock();
    ...
    /**
     * Returns a {@code java.util.Map} which is a simple mechanism for getting
     * the current revision.
     * <p/>
     * The returned {@code java.util.Map} is read-only and cannot be modified in
     * any way.
     */
