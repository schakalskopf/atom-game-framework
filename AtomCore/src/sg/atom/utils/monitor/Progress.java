...
/**
 * start -> end로 진행하는 진행상태값. addProgress()를 호출할 경우, 바로 값이 적용되지 않고 update()를 호출함에
 * 따라 처음에는 빨리, 목표치에 도달할 때는 천천히 증가한다. 그리고, 목표치가 end값과 같아지면, maxSpeed로 진행한다.
 *
 * @author mulova
 */
public class Progress implements IProgress {
    ...
    /**
     * update로 올라갈 수 있는 최대 값. 즉 addProgress(), setProgress()에 의해 결정된 값. end ==
     * 1일 경우에만 setProgress() == getProgress() 이다.
     *
     * @return
     */
    @Override
    ...
    /**
     * progress가 max이 되었을 때의 실제 값 여러개의 progress가 모여 하나의 progress가 될 때 사용한다. 예를
     * 들면 0.3, 0.3, 0.4 의 세개의 progress로 이루어 졌다고 할때, setLimit(0.3f)
     * setProgress(0.5f) => getProgress() = 0.15f setProgress(1f) =>
     * getProgress() = 0.3f 가 되고, 이후 setLimit(0.6f) setProgress(0.5f) =>
     * getProgress() = 0.15f setProgress(1f) => getProgress() = 0.3f 가 되고,
     *
     * @param max progress의 최대값. 0~1사이의 값을 가지며, setProgress(1) 일때 실제로 max값이 된다.
     */
    ...
    /**
     * 설정한 값이 바로 적용되지 않는다. 현재의 progress보다 작은 값일 경우 무시된다.
     */
    @Override
