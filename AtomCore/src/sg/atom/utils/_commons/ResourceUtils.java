...
import sg.atom.utils.io.ExtFileFilter;
...
public class ResourceUtils {
    ...
    public static HashMap<String, HashMap<String, File>> listFiles(String... classpaths) {
        ...
        for (int i = classpaths.length - 1; i >= 0; i--) {
            final String classpath = StringUtils.toDirectory(classpaths[i]);
            List<File> files = listFiles(new File(classpath), FileTypeFilters.ALL, null);
            for (File file : files) {
                final File dir = file.getParentFile();
                String key1 = StringUtils.toDirectory(dir.getAbsolutePath());
                String key2 = StringUtils.changeFileSeparator(file.getAbsolutePath());
                key1 = StringUtils.detachPrefix(key1, classpath);
                key2 = "/" + StringUtils.detachPrefix(key2, classpath);
    ...
    public static final java.awt.Image loadAWTImage(final String path) {
        return ResourceUtils.loadAWTImage(getClasspathResource(path));
    ...
    public static final String findResource(String imageLocation, String... classpaths) {
        String filename = StringUtils.getFileName(imageLocation);
        for (String cp : classpaths) {
            String path = StringUtils.concatPath(cp, filename);
    ...
    public static final String toRelativePath(String path, String... rootPaths) {
        if (currentClasspath == null) {
            ...
            for (int i = 0; i < currentClasspath.length; i++) {
                currentClasspath[i] = StringUtils.toDirectory(currentClasspath[i]);
            }
        }
        for (int i = 0; i < rootPaths.length; i++) {
            rootPaths[i] = StringUtils.toDirectory(rootPaths[i]);
        }

        path = StringUtils.changeFileSeparator(path);
    ...
    public static final URL getAnyResource(String path) {
        ...
        file = new File(StringUtils.concatPath(dir, path));
        if (file.exists()) {
            try {
                return file.toURI().toURL();
            } catch (MalformedURLException e2) {
    ...
    public static String getUserDir(String relativePath) {
        ...
        return StringUtils.concatPath(base, relativePath);
    }
