...
import sg.atom.core.context.AtomContext;
import sg.atom.core.context.DataManager;
 ...
 * via the call
 * {@code AtomContext.getDataManager().removeObject(ManagedObject)}.
 * <p>
...
public class ScalableList<E> extends AbstractList<E> implements Serializable,
	...
	headRef =
		AtomContext.getDataManager().createReference(
	...
	return AtomContext.getDataManager().createReference(t);
    }
	...
	if (obj instanceof Element) {
	    ...
	    if (delete) {
		AtomContext.getDataManager().removeObject(obj);
	    }
	    return tmp.getValue();
	}
	return obj;
	...
	AtomContext.getDataManager().markForUpdate(this);
	...
	AtomContext.getTaskManager().scheduleTask(clearTask);

	// Remove the dummy connectors
	DataManager dm = AtomContext.getDataManager();
	...
	DummyConnector(Node<E> reference) {
	    if (reference != null) {
		ref = AtomContext.getDataManager().createReference(reference);
	    }
	}
	...
	private TreeNode(ScalableList<E> list, TreeNode<E> parent,
	    ...
	    // Set up links
	    assert (child != null);
	    childRef = AtomContext.getDataManager().createReference(child);
	...
	private TreeNode(ScalableList<E> list) {
	    owner = AtomContext.getDataManager().createReference(list);
	...
	public void clear() {
	    ...
	    AtomContext.getDataManager().removeObject(this);
	    if (parent != null) {
		parent.clear();
	    }
	}
	...
	public void run() {
	    ...
	    if (doWork()) {
		AtomContext.getTaskManager().scheduleTask(this);
	    } else {
	...
	private boolean doWork() {
	    ...
	    AtomContext.getDataManager().markForUpdate(currentListNode);
	    ListNode<E> next;
	    ...
	    while (size > 0 && currentListNode != null &&
                   AtomContext.getTaskManager().shouldContinue()) {
		...
		if (entry instanceof Element) {
		    AtomContext.getDataManager().removeObject(entry);
		}
	    }
	...
	ScalableIterator(ScalableList<E> list, ListNode<E> startingNode) {
	    owner = AtomContext.getDataManager().createReference(list);
	    currentNode =
		    AtomContext.getDataManager().createReference(startingNode);
	...
	private boolean loadNextListNode() {
	    ...
	    if (next != null) {
		currentNode =
			AtomContext.getDataManager().createReference(next);
		cursor = 0;
		return true;
	    }
	    return false;
	...
	void doRemove() {
		...
		if (prev != null) {
		    currentNode =
			    AtomContext.getDataManager().createReference(prev);
	...
	private ListNode(TreeNode<E> parent) {
	    ...
	    parentRef = AtomContext.getDataManager().createReference(parent);
	}
	...
	ListNode(TreeNode<E> parent, int maxSize) {
	    ...
	    count = sublist.size();
	    subListRef = AtomContext.getDataManager().createReference(sublist);
	}
	...
	private void checkRemoveListNode(ScalableList<E> list) {
	    ...
	    AtomContext.getDataManager().removeObject(getSubList());
	    AtomContext.getDataManager().removeObject(this);
	}
	...
	boolean remove(Object obj) {
		...
		if (obj.equals(object)) {
			...
			AtomContext.getDataManager().removeObject(stored);
		    }
		    return true;
		}
	    }
