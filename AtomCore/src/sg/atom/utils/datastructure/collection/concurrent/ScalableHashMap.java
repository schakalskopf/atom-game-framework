...
import sg.atom.utils.datastructure.serializable.ManagedSerializable;
...
public class ScalableHashMap<K, V>
	...
	DataManager dataManager = AtomContext.getDataManager();
	...
	int hash = (key == null) ? 0x0 : hash(key.hashCode());
        ScalableHashMap<K, V> leaf = lookup(hash);
	AtomContext.getDataManager().markForUpdate(leaf);
	leaf.modifications++;
	...
	AtomContext.getDataManager().markForUpdate(this);
	modifications++;
	TaskManager taskManager = AtomContext.getTaskManager();
	if (isLeafNode()) {
	...
	for (int i = 0; i < table.length; i++) {
		...
		table[i] = next;
                count++;
		if (!AtomContext.getTaskManager().shouldContinue()) {
		    return count;
		}
	    }
	}
	 ...
	 */
	    ...
	    rootRef = AtomContext.getDataManager().createReference(root);
	    getNext();
	}
	...
	/** Returns the current leaf. */
	    ...
	    currentLeafRef = AtomContext.getDataManager().createReference(leaf);
	    return leaf;
	}
	 ...
	 */
	    ...
	    currentLeafRef =
		AtomContext.getDataManager().createReference(nextLeaf);
	...
	ManagedReference<ScalableHashMap<K, V>> nodeRef =
	    AtomContext.getDataManager().createReference(leftMost());
	while (nodeRef != null) {
