 ...
 */
package sg.atom.utils.datastructure.collection.immutable;
...
import sg.atom.utils.concurrent.IllegalConcurrentAccessException;
import sg.atom.utils.concurrent.Immutable;

/**
 * FreezableList is an implementation of the {@link List} interface that allows
 * freezing the list. Once the list has been frozen (using the method
 * {@link #freeze()}, no modification is allowed. However, the list can then
 * accessed using an {@link ImmutableList} instance that is multi-threading
 * safe.
 *
 * FreezableList is only safe for use in a single thread. It has performance
 * characteristics similar to {@link ArrayList}.
 *
...
public class FreezableList<T> extends AbstractList<T> {
    ...
    /**
     * Creates a new, empty FreezableList instance with the given initial
     * capacity. Specifying the capacity is useful for performance only, as
     * every increase of capacity is relatively costly. It does not have any
     * functional difference, as
     * <code>FreezableList</code> will increase its capacity automatically.
     *
     * @param contentType the super-class of the class members
    ...
    /**
     * Creates a new FreezableList instance with the given initial content. The
     * initial data collection will be copied (shallowly).
     *
     * @param contentType the super-class of the class members
     ...
     * @param immutableContent if true, all elements of the array are guaranteed
     * to be immutable. If false, some or all may be not
     */
    ...
    /**
     * Creates a new
     * <code>FreezableList</code> with the given elements
     *
    ...
    /**
     * Computes the recommended capacity of the internal array if actual length
     * of the list increases or decreases to the given length.
     *
    ...
    /**
     * Makes sure that this list has at least the given capacity. If not, the
     * size of the list will be increased. Please note that this method is only
     * useful for performance reasons. FreezableList will always automatically
     * increase its capacity if needed. The advantage of
     * <code>ensureCapacity</code> is that is can minimize the number of costly
     * resizing operations.
     *
    ...
    /**
     * Adjusts the capacity to the given new size, if it is not large enough
     * yet.
     *
    ...
    /**
     * Freezes the list. After the first invocation of
     * <code>freeze</code>, the list can not be modified anymore. All calls that
     * would modify the list will throw a {@link UnsupportedOperationException}.
     * You can call
     * <code>freeze</code> any number of times. <p> The method returns an
     * {@link ImmutableList}, which is a thread-safe representation of the
     * <code>FreezableList</code> at the time of freezing. The
     * <code>FreezableList</code> itself is never thread-safe, not even after
     * freezing. <p> Freezing can only work if all members of the list are
     * {@link Serializable} (or, even better, {@link Immutable}).
     *
     * @return the current content as <code>ImmutableList</code>
     * @throws IllegalConcurrentAccessException if one of the values was not
     * {@link Serializable}
     */
    ...
    /**
     * Adds the given element into the list at the given position. All existing
     * elements from the index to the end will be moved to the back. The list's
     * size increases by one and, if needed, the lists capacity will be
     * increased. By specifying the first index after the list's last element,
     * the new element can be placed into the list's back.
     *
     * @param index the index to add the element to. Must not be negative, and
     * not larger than the list's current size
    ...
    /**
     * Adds the content of the given collection into the list at the given
     * position. All existing elements from the index to the end will be moved
     * to the back. The list's size increases by the collection's size and, if
     * needed, the lists capacity will also be increased. By specifying the
     * first index after the list's last element, the new elements can be placed
     * into the list's back.
     *
     * @param index the index to add the new elements to. Must not be negative,
     * and not larger than the list's current size
     * @param c the collection of elements to add
     * @return true if the collection has at least one element, false for empty
     * collections
     * @throws IndexOutOfBoundsException if the index is not in the valid range
    ...
    /**
     * Adds the content of the given immutable list into this list at the given
     * position. All existing elements from the index to the end will be moved
     * to the back. This list's size increases by the collection's size and, if
     * needed, the lists capacity will also be increased. By specifying the
     * first index after this list's last element, the new elements can be
     * placed into the list's back.
     *
     * @param index the index to add the new elements to. Must not be negative,
     * and not larger than the list's current size
     ...
     *
     * @param index the index of the element to get. Must not be negative, and
     * smaller than the list's current size
    ...
    /**
     * Removes the element at the given index. All following elements will be
     * moved forward by one position, and the list's size decreased by one.
     *
     ...
     *
     * @param index the index of the element to set. Must not be negative, and
     * smaller than the list's current size
    ...
    /**
     * Returns an array that contains a copy of this list (shallow copies,
     * references only). If the given array has at least the length of this
     * list, the elements will be copied into it. Otherwise a new array will be
     * created whose length is the same as this list's.
     *
     * @param a the array to copy the data into, provided that it is large
     * enough
     * @return the copied array
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Removes the elements from the given start index (inclusive) to the given
     * end index (exclusive). If they are the same, no element will be removed.
     * All elements starting at the end index will be moved forward.
     *
     * @param fromIndex the first index of the elements to remove. Must not be
     * negative, and smaller than the list's current size
     * @param toIndex the first index that must not be removed. Must not be
     * negative, not larger than the list's current size, and not smaller than
     * the start index
