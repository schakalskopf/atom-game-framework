...
/**
 * ImmutableMap is a special map implementation that can not be modified after
 ...
 * keys and values. They will be copied at least once: the first time when you
 * create the ImmutableSet instance is being created, and additionally every
 * time you read them.
 *
...
public final class ImmutableMap<K, V> {
     ...
     * Keys and values that are not immutable (but only Serializable) will be
     * copied. Actors will be proxied.
     *
    ...
    /**
     * Returns a set of all keys in the map. Please note that it is both easier
     * and faster to use the iterator() method (or the {@link Iterable}
     * interface, to use the map keys in a for loop) than calling this method
     * only to iterate over all elements in the map.
     *
    ...
    /**
     * Returns the map value with the given key. Please note that if the value
     * is non-immutable, a copy of the value will be created and the result can
     * only be used in the Thread that invoked the method (unless you use proper
     * synchronization to read it in other threads).
     *
     * @param key the key to look up
     * @return the value with the given key, or null if not found
     * @throws ActorRuntimeException if the value is not immutable and can not
     * be copied/serialized
     */
    public V get(K key) {
        if (valuesImmutable) {
            return data.get(key);
    ...
    /**
     * Returns the content of the map as a Map. All elements that are not
     * Immutable or actors will be copied again.
     *
     ...
     * and all entries are equal. Calls {@link Object#equals(Object)} on all
     * entries.
     *
    ...
    /**
     * Returns the hash code of the map. Calls {@link Object#hashCode()} on
     * every entry.
     *
