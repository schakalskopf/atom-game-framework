...
/**
 * Fast FIFO queue implementation. Entries must extend FastQueue.Entry. Unlike
 * Java implementations, it can remove entries by reference in o(1) instead of
 * o(n). FastQueue is not thread-safe.
 *
...
public class FastQueue<T extends FastQueue.Entry> implements Iterable<T> {
    ...
    /**
     * The base class of all entries of FastQueue. A object can only be in one
     * FastQueue at a time. The result of adding an object that is already in
     * another FastQueue is undefined (but probably not good).
     */
    public static class Entry {

        protected Entry fqePrev, fqeNext; // prev/next element. Null if at begin/end of the queue
    }
    ...
    /**
     * Returns the first element of the queue and removes it. Returns null if
     * empty.
     *
     * @return the first element or null
     */
    ...
    /**
     * Removes the given entry from this FastQueue. Does nothing if the element
     * is in no list. Please note: do not remove an instance that is in another
     * FastQueue. The result is undefined.
     *
    ...
    /**
     * Checks whether the given entry is in a queue. Please note that this
     * function will not determine whether it is actually in this queue, or any
     * other (that would cause a o(n) complexity instead of o(1)).
     *
