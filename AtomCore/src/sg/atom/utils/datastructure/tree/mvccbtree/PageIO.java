...
/**
 * A structure containing a Page on disk. It's a byte[PageSize] plus a few more
 * details like the page offset on disk and a link to the next page.</br> As we
 * may need more than one Page to store some data, the PageIO are linked so that
 * the list of all the PageIO contain the full data.</br> The first PageIO
 * contains the size of the data.</br> Here is the logical structure of a PageIO
 * :
 * <pre>
 * For a first page :
 *
...
public class PageIO {
    ...
    /**
     * Get the NextPage value from the PageIO. If it's -1, there is no next
     * page<br/>
     *
     * @return the nextPage
     */
