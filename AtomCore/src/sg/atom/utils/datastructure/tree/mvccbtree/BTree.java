...
/**
 * A BTree interface, to be implemented by the PersistedBTree or the
 * InMemoryBTree
 *
...
public interface BTree<K, V> {
     ...
     * <br/> If the provided size is below 2, we will default to
     * DEFAULT_PAGE_SIZE.<br/> If the provided size is not a power of 2, we will
     * select the closest power of 2 higher than the given number<br/>
     *
    ...
    /**
     * Insert an entry in the BTree. <p> We will replace the value if the
     * provided key already exists in the btree.
     *
    ...
    /**
     * Delete the entry which key is given as a parameter. If the entry exists,
     * it will be removed from the tree, the old tuple will be returned.
     ...
     * value If the value is present, it will be deleted first, later if there
     * are no other values associated with this key(which can happen when
     * duplicates are enabled), we will remove the key from the tree.
     *
     * @param key The key for the entry we try to remove
    ...
    /**
     * Find a value in the tree, given its key. If the key is not found, it will
     * throw a KeyNotFoundException. <br/> Note that we can get a null value
     * stored, or many values.
     *
     ...
     * @throws IOException If we had an issue while accessing the underlying
     * file
     * @throws KeyNotFoundException If the revision does not exist for this
     * Btree
     */
     ...
     * key is not found, it will throw a KeyNotFoundException. <br/> Note that
     * we can get a null value stored, or many values.
     *
     * @param revision The revision for which we want to find a key
     * @param key The key we are looking at
     * @return The found value, or null if the key is not present in the tree
     * @throws KeyNotFoundException If the key is not found in the BTree
     * @throws IOException If there was an issue while fetching data from the
     * disk
     */
    ...
    /**
     * Checks if the BTree contains the given key with the given value for a
     * given revision
     *
    ...
    /**
     * Creates a cursor starting at the beginning of the tree, for a given
     * revision
     *
     * @param revision The revision we would like to browse
     * @return A cursor on the btree
     ...
     * @throws KeyNotFoundException If we can't find a rootPage for this
     * revision
     */
    ...
    /**
     * Flush the latest revision to disk. We will replace the current file by
     * the new one, as we flush in a temporary file.
     */
